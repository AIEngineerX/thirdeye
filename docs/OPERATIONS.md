...
| No metrics endpoint | The `auth_tokens.rate_bucket` column and the `signals` table are the canonical sources — run scheduled SQL queries against them. Prometheus/OTEL exporters are a v2 enhancement. |
...
| Signals never update or close | Confirm the `graphile-worker` process is running and check `[signals-refresh]` log lines. A price-source outage logs and is retried next tick; market cap comes from DexScreener (free, no key). |
