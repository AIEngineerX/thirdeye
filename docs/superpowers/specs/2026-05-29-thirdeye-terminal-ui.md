...
- **`PriceChart`** (token) — candlesticks from OHLCV.
  ...
  - **Route:** a **dedicated** `GET /api/token/:mint/ohlcv` behind `requireAuth` but **not** the Helius `scanTokenLimit` (it's a cheap cached read, not a scan). A **60s in-process TTL cache** in front — adequate for a single-user personal instance on the ST free tier; persistent caching is out of scope (revisit only if hosted multi-user).
...
|---|---|---|
| Token OHLCV (candles) | Solana Tracker `/chart/{mint}` | `tokenChart()` client method (against a captured fixture) + dedicated `GET /api/token/:mint/ohlcv` (`requireAuth`, **no** scan limiter, 60s in-process cache) + `apps/web` `lib/ohlcv-types.ts` mirror |
