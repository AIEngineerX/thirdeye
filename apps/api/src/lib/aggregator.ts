...
// Drizzle's db.execute<T> requires T to satisfy Record<string, unknown>;
// intersect with it so the named row shapes still bind to the generic.
type Pulse24hRow = {
  scans: number;
  checks: number;
  new_clusters: number;
...
type BucketRow = { bucket: number; count: number } & Record<string, unknown>;
...
export async function pulse24h(db: DbClient): Promise<Pulse24h> {
  const r = await db.execute<Pulse24hRow>(sql`
    SELECT
  ...
  `);
  const row = (r as unknown as Pulse24hRow[])[0];
  return {
    scans: row?.scans ?? 0,
    checks: row?.checks ?? 0,
...
export async function allTime(db: DbClient): Promise<AllTime> {
  const r = await db.execute<AllTimeRow>(sql`
    SELECT
  ...
  `);
  const row = (r as unknown as AllTimeRow[])[0];
  return {
...
export async function riskDist(db: DbClient): Promise<RiskDist> {
  ...
  let total = 0;
  for (const row of r as unknown as BucketRow[]) {
...
export async function heatmap(db: DbClient): Promise<Heatmap> {
  const r = await db.execute<HeatmapRow>(sql`
    SELECT mint, symbol, name, risk_pct, verdict, scanned_at
    FROM token_scans
    ORDER BY scanned_at DESC
    LIMIT 20
  `);
  const items: HeatmapItem[] = (r as unknown as HeatmapRow[]).map((row) => ({
    mint: row.mint,
    symbol: row.symbol,
    name: row.name,
